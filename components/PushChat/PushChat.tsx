...
const PushChat: React.FC<PushChatProps> = ({ selectedAddress, onClose }) => {
  ...
  const currentUserAddress = "0x98692B795D1fB6072de084728f7cC6d56100b807";
        ...
        (payload) => {
          ...
          setMessages((prev) => {
            ...
            if (selectedAddress === "Global Chat") {
              ...
              if (
                !newMessage.is_global &&
                ((newMessage.sender_address === currentUserAddress &&
                  newMessage.receiver_address === selectedAddress) ||
                  (newMessage.sender_address === selectedAddress &&
                    newMessage.receiver_address === currentUserAddress))
              ) {
                return [...prev, newMessage];
              }
            }
    ...
    try {
      ...
      if (selectedAddress === "Global Chat") {
        ...
        query = query
          .eq("is_global", false)
          .or(
            `sender_address.eq.${currentUserAddress},receiver_address.eq.${currentUserAddress}`
          )
          .or(
            `sender_address.eq.${selectedAddress},receiver_address.eq.${selectedAddress}`
          );
      }
          ...
          : data.filter(
              (msg) =>
                (msg.sender_address === currentUserAddress &&
                  msg.receiver_address === selectedAddress) ||
                (msg.sender_address === selectedAddress &&
                  msg.receiver_address === currentUserAddress)
            );
  ...
  const handleSend = async () => {
    ...
    try {
      const newMessage = {
        ...
        receiver_address:
          selectedAddress === "Global Chat" ? null : selectedAddress,
